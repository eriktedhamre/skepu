...
void findLowest(std::vector<Record> &records,skepu::Vector<float> distances,int numRecords,int topN);
void printUsage();
...
* Kernel
* Executed on GPU
* Calculates the Euclidean distance from each record in the database to the target position (lat,lng)
*/
...
/** 
* I assume each thread will calculate the distance to the point (lat, lang) from one element in d_locations.
* Thus indexing on it should be fine
* is powf of interest to enable in user functions
*/
float skepu_euclid(skepu::Index1D index, skepu::Vec<LatLong> latLong, float lat, float lng)
{
    return (float)sqrt((lat-latLong(index.i).lat)*(lat-latLong(index.i).lat) + (lng-latLong(index.i).lng)*(lng-latLong(index.i).lng));
}
...
int main(int argc, char* argv[])
{
    ...
    skepu::Vector<LatLong> skepu_locations(numRecords);

    for (size_t i = 0; i < locations.size(); i++)
    {
        LatLong elem;
        elem.lat = locations[i].lat;
        elem.lng = locations[i].lng;
        skepu_locations[i] = elem;
    }
    

    //Pointers to host memory
	//float *distances;
    skepu::Vector<float> distances(numRecords);
    ...
    */
    ...
    auto instance = skepu::Map<0>(skepu_euclid);
    instance(distances, skepu_locations, lat, lng);
